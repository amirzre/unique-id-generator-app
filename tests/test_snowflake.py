...
class TestSnowflakeIDGenerator:
    ...
    def test_sequential_ids_within_same_millisecond(self):
        ...
        ids = [
            ...
            for _ in range(SnowflakeIDGenerator.MAX_SEQUENCE + 1)
        ]
        assert len(set(ids)) == len(
            ids
        ), "IDs within the same millisecond should be unique."
    ...
    def test_thread_safety(self):
        ...
        assert (
            len(ids) == 10000
        ), "All generated IDs should be unique even in a multithreaded context."
    ...
    def test_edge_datacenter_and_machine_id(self):
        ...
        generator = SnowflakeIDGenerator(
            SnowflakeIDGenerator.MAX_DATACENTER_ID, SnowflakeIDGenerator.MAX_MACHINE_ID
        )
        id1 = generator.generate_id()
        assert (
            id1 > 0
        ), "Generated ID should be positive with max datacenter_id and machine_id."
    ...
    def test_timestamp_with_custom_epoch(self):
        ...
        assert (
            abs(timestamp_part - current_time) < 10
        ), "Timestamp should match the current time."
    ...
    def test_no_collision_in_multithreaded_environment(self):
        ...
        for _ in range(num_threads):
            thread = threading.Thread(
                target=generate_ids, args=(self.generator, num_ids, results)
            )
        ...
        assert len(unique_results) == len(
            results
        ), "There should be no duplicate IDs in a multithreaded environment."
